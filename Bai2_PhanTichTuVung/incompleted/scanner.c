...
void skipBlank() {
	while (currentChar != -1 && charCodes[currentChar] == CHAR_SPACE){
    	// printf("%d %d \n", currentChar, charCodes[currentChar]);
		readChar();
	}
}
...
void skipComment() {
	while(1){
		if(currentChar == -1){
			...
			if(currentChar == -1){
				error(ERR_ENDOFCOMMENT, lineNo, colNo);
			}else if(charCodes[currentChar] == CHAR_RPAR){	// The currentChar is right parenthesis
				// End of comment
				// printf("%c %d \n", currentChar, charCodes[currentChar]);
				readChar();
				return;
...
Token* readIdentKeyword(void) {
	int count = 0;
	// Constructor 1 token with type identifier
	Token *token = makeToken(TK_IDENT, lineNo, colNo);

	while(charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT){
		// string la chuoi duoc luu tru
	...
	case CHAR_LPAR:
		...
		switch(charCodes[currentChar]){
			case CHAR_PERIOD:
				...
				return token;
			case CHAR_TIMES:
				// This is a comment -> free token return then skip comment
				free(token);
				skipComment();
				return getToken();
			case CHAR_SPACE:
